...
This chapter describes Quattro integrations, not a promise that every upstream
GUI is present in the ARM core image. Nautilus, Chromium, Neovim, and the
essential desktop tools are declared; heavy/proprietary applications are
profile-dependent and physical Pi/M2 behavior remains unverified. The
upstream _Install_ menu does not perform Pacman/AUR installation on OmixOS.
...
Signal is optional and not part of the Pi core image. Add it through a
compatible NixOS/profile overlay before expecting a launcher; the upstream
first-run _Install > Service_ behavior is not available.
