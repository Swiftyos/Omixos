...
The Quickshell bar is shared across OmixOS targets. Optional services and
widgets are profile- or overlay-dependent; upstream _Install → Service_ entries
must not be read as Arch package installation instructions.
...
| Omarchy update | Explain/run the Nix update flow | — | — |
...
Two more widgets appear only when the matching service is present in the
selected NixOS profile or overlay, and both do more than report status.
...
This panel is functional when the NixOS Tailscale service adapter is enabled;
the Pi image's physical networking and tailnet acceptance are still pending.
