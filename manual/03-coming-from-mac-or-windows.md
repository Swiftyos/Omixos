...
Your Spotlight or Raycast or Start-menu reflex becomes `Super + Space`. That
opens the Omarchy menu, which launches apps, changes user settings, captures
the screen, and exposes the supported command surface. System software changes
still belong in NixOS/Home Manager configuration. Start typing to filter.
There's also a dedicated apps-only menu on `Super + Alt + Space`. See
...
| Time Machine (for the system) | NixOS generations and explicit [rollback](47-system-snapshots.md) |
| App Store / downloading an installer | Declarative flake/profile changes — see [other packages](29-other-packages.md) |
