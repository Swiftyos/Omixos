...
Availability follows the selected OmixOS profile. The Pi 4 `core` image
includes the essential terminal/runtime tools; workstation-only TUIs (such as
container tooling) are not guaranteed. The upstream _Install > TUI_ action is
not an Arch package installer here.
