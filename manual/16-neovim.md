...
Neovim is part of the OmixOS **core** profile on ARM64; it is not an optional
Arch package or an AUR install. The complete editor configuration is seeded
into writable user state where applicable, while the runtime itself remains
immutable. The headless ARM system test verifies the installed `nvim`
executable.
...
The Omarchy quattro source ships a complete Neovim setup built on
[LazyVim](https://www.lazyvim.org/). OmixOS packages the editor in the core
profile; use Home Manager/user state for local configuration rather than
installing `omarchy-nvim` through Pacman.
