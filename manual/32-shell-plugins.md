...
The plugin model is one of the Quattro behaviors retained by OmixOS. Built-ins
come from the immutable runtime; user plugins live under writable
`~/.config/omarchy/plugins`. Plugin commands do not install system packages and
are not a substitute for declarative NixOS services. Plugin/session behavior is
covered in the ARM VM; physical Pi/M2 acceptance remains pending.
