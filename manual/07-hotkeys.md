...
| `Super + Ctrl + X` | Start/stop dictation (requires a profile with Voxtype) |
| `F9` | Push-to-talk dictation (requires a profile with Voxtype) |
...
Extra background images live in `~/.config/omarchy/backgrounds/<theme name>`;
copy them there directly because the upstream style installer is not a package
operation on OmixOS.
